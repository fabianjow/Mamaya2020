...
if (isset($_SESSION["Cart"])) {
	...
	//$qry = "SELECT p.ProductID FROM Product p
	//		INNER JOIN ShopCartItem sci ON p.ProductID=sci.ProductID
	//		WHERE ProductImage=?";
	//$stmt = $conn->prepare($qry);
	...
	if ($result->num_rows > 0) {
		...
		while ($row = $result->fetch_array()) {
			...
			$MainContent .= "<tr>";
			//$MainContent .= "<td>$row[ProductImage]</td>";
			...
			for($i = 1; $i <= 10; $i++){ // To populate drop-down list from 1 to 10
				if($i == $row["Quantity"]){
					// Select the drop-down list item with value same as the qunatity of purchase
					$selected = "selected";
				}
				else{

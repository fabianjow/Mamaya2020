	 ...
	 foreach($_SESSION["Items"] as $key=>$item){
		...
		if($result->num_rows > 0){
			...
			if($row["Quantity"] < $item["quantity"]){
				$MainContent .= "Product $item[productId] : $item[name] is out of stock!";
				$MainContent .= "<br />";
				$MainContent .= "Please return to shopping cart to amend you purchase.";
				$MainContent .= "<br />";
			...
			$MainContent .= "Product $item[productId] : $item[name] is out of stock!";
			$MainContent .= "<br />";
			$MainContent .= "Please return to shopping cart amend your purchase.";
			$MainContent .= "<br />";
			include("MasterTemplate.php");
			exit;
		}
...
{	
	   ...
	   "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) 
	{
		...
		foreach($_SESSION["Items"] as $key=>$cartItem){
			// "ii" - 2 integers
			$stmt->bind_param("ii", $cartItem["quantity"], $cartItem["productId"]);
			$stmt->execute();
		}
